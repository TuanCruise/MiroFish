...
- Enters command waiting mode instead of closing environment immediately after simulation
...
# This is to fix OASIS third-party libraries not specifying encoding when reading files
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Reconfigure standard output streams to UTF-8 (Fixes console Chinese garbled text)
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        """
        # Set default encoding only for text mode (non-binary) and when no encoding is specified
...
# Load .env file from project root (contains LLM_API_KEY config, etc.)
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai warnings about max_tokens (we intentionally do not set max_tokens, letting model decide)"""
...
# Add filter immediately upon module load, ensuring it takes effect before camel code executes
...
def init_logging_for_simulation(simulation_dir: str):
    ...
    # Clean up old log dir (if exists)
...
# Twitter available actions (excludes INTERVIEW, INTERVIEW can only be manually triggered via ManualAction)
TWITTER_ACTIONS = [
...
# Reddit available actions (excludes INTERVIEW, INTERVIEW can only be manually triggered via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None/Not specified: Interview on both platforms simultaneously, returning unified results
        ...
        # Platform not specified: Interview both simultaneously
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        for interview in interviews:
            ...
            if item_platform == "twitter":
                ...
                # Platform not specified: Interview both
        ...
        # Process interviews for Reddit platform
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows displaying real entity names in actions.jsonl instead of "Agent_0" codes
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from database and supplement with full context info
    
    Args:
        db_path: Database file path
        last_rowid: Max rowid read last time (using rowid because created_at formats differ across platforms)
        agent_names: agent_id -> agent_name mapping
        
    Returns:
        (actions_list, new_last_rowid)
        - actions_list: Action list, each containing agent_id, agent_name, action_type, action_args (with context)
    ...
    try:
        ...
        # This avoids the created_at format discrepancy (Twitter uses int, Reddit uses datetime string)
        cursor.execute("""
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, keep only key fields (keep full content, do not truncate)
            simplified_args = {}
            ...
            # Supplement context info (post content, user name, etc.)
...
def _enrich_action_context(
    ...
    """
    Supplement action with context info (post content, username, etc.)
    ...
    try:
        ...
        # Quote post: supplement original post content, author, and quote comment
            ...
            # Get quote content of the quoted post
            if new_post_id:
                cursor.execute("""
        ...
        # Create comment: supplement post info being commented on
        ...
        # Failing to supplement context does not affect main flow
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    - Base config: LLM_API_KEY, LLM_BASE_URL, LLM_MODEL_NAME
    - Boost config (optional): LLM_BOOST_API_KEY, LLM_BOOST_BASE_URL, LLM_BOOST_MODEL_NAME
    
    If boost LLM is configured, parallel simulation can assign different API providers
    to different platforms to increase concurrency.
    ...
    # Choose LLM based on args and config
    if use_boost and has_boost_config:
        # Use boost config
        llm_api_key = boost_api_key
        llm_base_url = boost_base_url
        llm_model = boost_model or os.environ.get("LLM_MODEL_NAME", "")
        config_label = "[Boost_LLM]"
    else:
        # Use base config
        llm_api_key = os.environ.get("LLM_API_KEY", "")
        llm_base_url = os.environ.get("LLM_BASE_URL", "")
        llm_model = os.environ.get("LLM_MODEL_NAME", "")
        config_label = "[Base_LLM]"
    
    # If no model name in .env, fallback to config
    if not llm_model:
...
async def run_twitter_simulation(
    ...
    total_actions = 0
    last_rowid = 0  # Track max rowid processed in DB (use rowid to avoid created_at format issues)
    ...
    for round_num in range(total_rounds):
        ...
        if not active_agents:
            # Log round end even if no active agents (actions_count=0)
            if action_logger:
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM config (if available, otherwise fallback to base config)
...
async def main():
    ...
    # Create shutdown event at start of main function to ensure whole program responds to exit signal
    ...
    # Initialize logging config (disable OASIS logging, clean old files)
    ...
    log_manager.info(f"  - Total simulation hours: {total_hours}h")
    log_manager.info(f"  - Minutes per round: {minutes_per_round}m")
    ...
    # Check if entering wait for command mode
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering command waiting mode - Environments kept running")
...
def setup_signal_handlers(loop=None):
    ...
    When termination signal received, need to:
    1. Notify asyncio loop to exit wait
    2. Allow program to properly clean up resources (close database, environments, etc.)
    3. Then exit
    """
