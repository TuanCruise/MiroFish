...
This script reads parameters from a configuration file to execute a simulation, fully automated.

Features:
- Does not close the environment immediately after simulation completes, enters command waiting mode
...
# Load .env file from the project root (contains LLM_API_KEY and other configs)
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai warnings about max_tokens (we intentionally do not set max_tokens so the model decides)"""
    
    def filter(self, record):
        # Filter out max_tokens warning logs
        if "max_tokens" in record.getMessage() and "Invalid or missing" in record.getMessage():
            return False
        return True


# Add filter immediately at module load time, ensure it takes effect before camel code executes
logging.getLogger().addFilter(MaxTokensWarningFilter())


def setup_oasis_logging(log_dir: str):
    """Configure logging for OASIS, using fixed names for log files"""
...
class IPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str) -> bool:
        ...
        try:
            ...
            self._send_response(command_id, {"status": "completed", "result": result})
            ...
            self._send_response(command_id, {"status": "failed", "error": error_msg})
            return False
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict]) -> bool:
        ...
        try:
            ...
            if not actions:
                self._send_response(command_id, {"status": "failed", "error": "No valid Agents"})
                return False
            ...
            self._send_response(command_id, {
                "status": "completed",
                "result": {
    ...
    async def process_commands(self) -> bool:
        ...
        if command_type == CommandType.INTERVIEW:
            ...
            self._send_response(command_id, {"status": "completed", "result": {"message": "Environment closing soon"}})
            return False
        
        else:
            self._send_response(command_id, {"status": "failed", "error": f"Unknown command type: {command_type}"})
            return True
...
class TwitterSimulationRunner:
    ...
    def _create_model(self):
        ...
        Unify using configuration from .env file in the project root (highest priority):
        - LLM_API_KEY: API key
        - LLM_BASE_URL: API base URL
        - LLM_MODEL_NAME: Model name
        """
        # Prioritize loading configuration from .env
    ...
    async def run(self, max_rounds: int = None):
        ...
        print(f"  - Minutes per round: {minutes_per_round} mins")
...
async def main():
    ...
    # Init log config (fixed filenames, clean old logs)
...
def setup_signal_handlers():
    """
    Setup signal handlers to ensure graceful exit on SIGTERM/SIGINT
    Gives program a chance to cleanup resources (close DB, environment, etc)
    """
