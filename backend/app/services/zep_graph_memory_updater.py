...
class ZepGraphMemoryUpdater:
    ...
    Monitors simulation actions log files to dynamically update agent activities to Zep graph in real time.
    Groups by platform and batches to Zep after accumulating BATCH_SIZE activities.
    
    All meaningful actions are updated to Zep, and action_args includes full context info:
    - Original post content for like/dislike
    - Original post content for repost/quote
    - Target username for follow/mute
    - Original comment content for like/dislike
    """
    
    # Batch sending size (how many items to accumulate per platform before sending)
    BATCH_SIZE = 5
    ...
    # Sending interval (seconds) to prevent sending requests too fast
    SEND_INTERVAL = 0.5
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # Activity buffer grouped by platform (accumulates up to BATCH_SIZE before sending)
        ...
        self._total_sent = 0        # Total successfully sent batches to Zep
        self._total_items_sent = 0  # Total successfully sent activities to Zep
        self._failed_count = 0      # Total failed batches
        self._skipped_count = 0     # Total activities skipped (DO_NOTHING)
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args includes full context parameters (e.g., original post content, username, etc.).
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        """
        Send batch activities to Zep graph (merged into a single text)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "batches_sent": self._total_sent,            # Total successfully sent batches
            "items_sent": self._total_items_sent,        # Total successfully sent activities
            "failed_count": self._failed_count,          # Total failed batches
            "skipped_count": self._skipped_count,        # Total skipped activities (DO_NOTHING)
...
class ZepGraphMemoryManager:
    ...
    # Flag to prevent multiple calls to stop_all
