...
class SimulationManager:
    ...
    def prepare_simulation(
        ...
        3. Use LLM to intelligently generate simulation config params (time, activity level, posting frequency, etc.)
        ...
        try:
            ...
            if filtered.filtered_count == 0:
                ...
                state.error = "No matched entities found, please check if the graph is properly built"
                self._save_simulation_state(state)
                return state
            ...
            # Reddit is already saved real-time during generation, save again here to ensure completeness
            if progress_callback:
                progress_callback(
            ...
            # ========== Stage 3: LLM intelligently generate simulation config ==========
            if progress_callback:
                progress_callback(
                    "generating_config", 0, 
            ...
            if progress_callback:
                progress_callback(
                    ...
                    "Calling LLM to generate configs...",
                    current=1,
                    total=3
                )
