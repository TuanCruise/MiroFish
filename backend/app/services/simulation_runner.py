...
class SimulationRunState:
    ...
    # Recent actions (for real-time display in frontend)
    ...
    def to_detail_dict(self) -> Dict[str, Any]:
        """Contains detailed info including recent actions"""
        result = self.to_dict()
...
class SimulationRunner:
    ...
    Responsibilities:
    1. Run OASIS simulation in the background process
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Maximum simulation rounds (optional, for truncating overlong simulations)
        ...
        Args:
            ...
            max_rounds: Maximum simulation rounds (optional, for truncating overlong simulations)
        ...
        try:
            ...
            # Create main log file to prevent process hang due to stdout/stderr pipe buffer full
            ...
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+ support, all open() defaults to UTF-8
            ...
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group, ensure all related processes can be terminated when server closes
            )
            
            # Save file handles for closing later
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        # New log structure: separate action logs by platform
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determine whether a platform is enabled by checking if corresponding actions.jsonl file exists
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        """
        Cross-platform termination of process and its sub-processes
    ...
    def _read_actions_from_file(
        ...
        Args:
            ...
            default_platform: Default platform (used when action record does not have a platform field)
    ...
    def get_all_actions(
        ...
        """
        Get complete action history for all platforms (without pagination)
    ...
    def get_timeline(
        ...
        Returns:
            Aggregated info for each round
        """
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Note: Will not delete configuration file (simulation_config.json) and profile files
        ...
        logger.info(f"Cleanup of simulation logs completed: {simulation_id}, deleted files: {cleaned_files}")
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check if there is content to clean up (avoid empty process printing useless logs)
            ...
            try:
                if process.poll() is None:  # Process is still running
                    ...
                    try:
                        ...
                        # Process might not exist anymore, try direct termination
                        try:
                            process.terminate()
    ...
    def register_cleanup(cls):
        ...
        Called on Flask application startup to ensure all simulation processes are cleaned up upon server shutdown
        """
        ...
        # WERKZEUG_RUN_MAIN=true means it's the reloader subprocess
        # If not debug mode, this env var won't exist, and cleanup still needs to be registered
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            # Call original signal handlers to allow Flask to exit normally
        ...
        try:
            ...
            logger.warning("Failed to register signal handler (not in main thread), using atexit only")
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specific platform (optional)
                ...
                - None: Interview both platforms during dual-platform simulation, returns combined results
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by platform of each interview item)
                - "twitter": Default only interview Twitter platform
                - "reddit": Default only interview Reddit platform
                - None: Interview both platforms for each Agent during dual-platform simulation
            timeout: Timeout duration (seconds)
    ...
    def close_simulation_env(
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Close environment command sent (wait for response timed out, environment might be closing)"
            }
    ...
    def get_interview_history(
        ...
        if platform in ("reddit", "twitter"):
            ...
            # Query both platforms if no platform specified
        ...
        # If querying multiple platforms, limit total amount
