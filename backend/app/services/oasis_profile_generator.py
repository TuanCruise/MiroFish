...
Convert entities from Zep graph into the Agent Profile format required by the OASIS simulation platform

Optimization Improvements:
1. Call Zep retrieval function to secondarily enrich node information
...
class OasisAgentProfile:
    ...
    def to_reddit_format(self) -> Dict[str, Any]:
        ...
        profile = {
            ...
            "username": self.user_name,  # OASIS lib requires field name to be username (without underscore)
            "name": self.name,
            "bio": self.bio,
...
class OasisProfileGenerator:
    ...
    Convert entities from Zep graph into the Agent Profile required by OASIS simulation
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep graph hybrid search feature to retrieve rich information related to the entity
        
        Zep doesn't have a built-in hybrid search interface, we need to search edges and nodes separately and combine the results.
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        if entity.related_edges:
            relationships = [] # Initialize relationships list here
            for edge in entity.related_edges:  # No limit on quantity
                ...
                if fact:
                    ...
                    if getattr(edge, 'source_node_uuid', None) == entity.uuid:
        ...
        if entity.related_nodes:
            related_info = [] # Initialize related_info list here
        ...
        if zep_results.get("facts"):
            ...
            if new_facts:
                context_parts.append("### Zep Retrieved Facts Info\n" + "\n".join(f"- {f}" for f in new_facts[:15]))
    ...
    def _generate_profile_with_llm(
        ...
        - Individual entities: generate concrete character settings
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # Simple check: if there is no comma or closing brace after the last quote, the string might be truncated
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        if json_match:
            ...
            try:
                ...
                # 5. If it still fails, try more aggressive fixing
                try:
        ...
        logger.warning(f"JSON fixing failed, returning basic structure")
        return {
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for public opinion simulation, closely replicating existing real-world scenarios. You must return a valid JSON format, and all string values must not contain unescaped newline characters. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, maximizing realism based on real-world scenarios.
...
2. persona: Detailed persona description (2000 words of plain text), including:
   ...
   - Background (key experiences, connection to events, social relations)
   ...
   - Stance/Views (attitude towards topics, factors that might trigger anger/emotion)
   - Unique Traits (catchphrases, special experiences, hobbies)
   - Personal Memory (important part of persona, must describe this individual's ties to the event and their past actions/reactions within the event)
...
Important:
- All field values must be strings or numbers, avoid using newline characters
- 'persona' must be a coherent paragraph of text description
- Use English
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account persona for an institution/group entity, maximizing realism based on real-world scenarios.
...
1. bio: Official account bio, 200 characters, professional and decent
2. persona: Detailed account persona description (2000 words of plain text), including:
   - Institution Basic Info (official name, nature of institution, founding background, main functions)
   - Account Positioning (account type, target audience, core functions)
   - Speech Style (language characteristics, common phrases, taboo topics)
   - Publishing Content Features (content types, publishing frequency, active time periods)
   - Stance/Attitude (official stance on core topics, approach to handling controversies)
   - Special Notes (representative group characteristics, operational habits)
   - Institution Memory (important part of persona, must describe this institution's ties to the event and their past actions/reactions within the event)
3. age: Fixed at 30 (virtual age of institutional account)
4. gender: Fixed at "other" (institutional accounts use "other" indicating non-personal)
...
8. interested_topics: Array of focused fields

Important:
- All field values must be strings or numbers, no null values allowed
- 'persona' must be a coherent paragraph of text description, avoid newlines
- Use English
- 'age' must be integer 30, 'gender' must be string "other"
- The institutional account's speech should match its identity and positioning"""
    ...
    def _generate_profile_rule_based(
        ...
        if entity_type_lower in ["student", "alumni"]:
            ...
            return {
                ...
                "age": 30,  # Virtual age of institutional account
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            realtime_output_path: File path for real-time writing (if provided, writes once per generation)
        ...
        def _save_realtime(current_profiles):
            """Save currently generated profiles to file in real-time"""
        ...
        def _generate_worker(idx, entity):
            ...
            try:
                ...
                self._print_profile_to_console(profile)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            # Submit all tasks
            future_to_entity = {
                executor.submit(_generate_worker, idx, entity): (idx, entity)
    ...
    def _print_profile_to_console(self, profile: OasisAgentProfile):
        ...
        output_lines = [
            f"\n{separator}",
            f"[Generated] {profile.name} ({profile.source_entity_type})",
            f"{separator}",
        ...
        # Only output to console (to avoid duplication, logger no longer outputs full content)
        print(output)
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter Profiles to CSV format (complies with OASIS official requirements)
    ...
    def _normalize_gender(self, gender: Optional[str]) -> str:
        """
        Standardize gender field to the English format required by OASIS
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Use the same format as to_reddit_format() to ensure OASIS can read it correctly.
        Must contain user_id field, this is the key for OASIS agent_graph.get_agent() matching!
        ...
        for idx, profile in enumerate(profiles):
            ...
            item = {
                ...
                # OASIS required fields - ensure they all have default values
