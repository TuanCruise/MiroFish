...
Reads nodes from Zep graph, filtering nodes that match predefined entity types
"""
...
class ZepEntityReader:
    ...
    2. Filter nodes that match predefined entity types (Labels are not just Entity)
    ...
    def _call_with_retry(
        ...
        Args:
            func: Function to execute (lambda or callable without parameters)
    ...
    def filter_defined_entities(
        ...
        Filtering logic:
        - If the node's Labels only contain "Entity", this entity does not match our predefined types, skip it
        - If the node's Labels contain labels other than "Entity" and "Node", it matches predefined types, keep it
        ...
        # Build mapping of node UUID to node data
        ...
        logger.info(f"Filtering completed: {total_count} total nodes, {len(filtered_entities)} match conditions, "
    ...
    def get_entity_with_context(
        ...
        try:
            ...
            # Get all nodes for associated lookups
            all_nodes = self.get_all_nodes(graph_id)
            node_map = {n["uuid"]: n for n in all_nodes}
