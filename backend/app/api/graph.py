...
def reset_project(project_id: str):
    """
    Reset project status (used to rebuild graph)
    """
    project = ProjectManager.get_project(project_id)
...
# ============== API 1: Upload file and generate ontology ==============

@graph_bp.route('/ontology/generate', methods=['POST'])
def generate_ontology():
    """
    API 1: Upload file, process and generate ontology definition
    ...
    try:
        ...
        print(f"DEBUG: Exception in generate_ontology: {e}")
...
def build_graph():
    ...
    try:
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is building, please do not submit repeatedly. To force rebuild, add force: true",
