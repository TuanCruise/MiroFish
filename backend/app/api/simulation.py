...
# Adding this prefix prevents Agents from calling tools, forcing direct text responses
INTERVIEW_PROMPT_PREFIX = "Combining your persona, all past memories, and past actions, reply to me directly with text without calling any tools:"
...
def get_graph_entities(graph_id: str):
    ...
    Only return nodes matching predefined entity types (nodes where Labels are not just 'Entity')
...
def create_simulation():
    ...
    Request (JSON):
        {
            ...
            "graph_id": "mirofish_xxxx",    // Optional, defaults to project graph_id
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Project graph has not been built, please call /api/graph/build first"
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Runtime scripts (run_*.py) are kept in backend/scripts/ directory and are not copied to the simulation directory anymore.
    ...
    # Required files list (excluding scripts, scripts are located in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # - ready: Preparation done, ready to run
        # - preparing: If config_generated=True, it means done
        # - running: Running, means preparation was done long ago
        # - completed: Run completed, means preparation was done long ago
        ...
        if status in prepared_statuses and config_generated:
            ...
            # If status is preparing but files are complete, automatically update to ready
            if status == "preparing":
                try:
                    ...
                    logger.info(f"Automatically updating simulation status: {simulation_id} preparing -> ready")
                    status = "ready"
                except Exception as e:
...
def prepare_simulation():
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Preparation task started|Existing preparation done",
                "already_prepared": true|false    // Whether it is already prepared
            }
        }
    """
    import threading
    ...
    try:
        ...
        logger.info(f"Starting processing /prepare request: simulation_id={simulation_id}, force_regenerate={force_regenerate}")
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Existing preparation is done, no need to regenerate",
        ...
        try:
            ...
            # Failure does not affect subseqent flow, background task will refetch
...
def get_prepare_status():
    ...
    Supports two querying methods:
    1. Query ongoing task progress by task_id
    2. Check if existing preparation work is done by simulation_id
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "prepare_info": {...}            // Detailed info when prep is complete
            }
        }
    """
    ...
    try:
        ...
        # If simulation_id is provided, check if preparation is done first
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Existing preparation work completed",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
        ...
        if not task:
            # Task does not exist, but if there is simulation_id, check if prepared
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Iterate through the reports directory to find the report matching the simulation_id.
    If there are multiple, return the latest one (sorted by created_at)
    ...
    try:
        ...
        # Sort descending by creation time, return the latest
...
def get_simulation_history():
    ...
    Used for displaying historical projects on the home page. Returns a simulation list containing rich information like project name, description, etc.
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get run status (read the actual number of rounds set by the user from run_state.json)
            run_state = SimulationRunner.get_run_state(sim.simulation_id)
            if run_state:
                sim_dict["current_round"] = run_state.current_round
                sim_dict["runner_status"] = run_state.runner_status.value
                # Use the total_rounds set by the user, if absent, use the recommended rounds
            ...
            # Get the file list of the associated project (max 3)
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Get Agent Profile for simulation in real-time (used for viewing progress during generation)
    ...
    - Returns extra metadata (e.g., file modification time, is generating flag, etc.)
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"Failed to read profiles file (might be writing): {e}")
                profiles = []
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Get simulation config in real-time (used for viewing progress during generation)
    
    Differences from /config API:
    - Reads file directly, bypasses SimulationManager
    - Suitable for real-time viewing during generation
    - Returns extra metadata (e.g., file modification time, is generating flag, etc.)
    - Can return partial information even if config is not fully generated
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"Failed to read config file (might be writing): {e}")
                config = None
        ...
        # Extract some key statistics if config exists
        if config:
...
def get_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": f"Simulation config does not exist, please call /prepare API first"
            }), 404
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return jsonify({
                "success": False,
                "error": "Config file does not exist, please call /prepare API first"
            }), 404
...
# ============== Profile Generation API (used independently) ==============
...
def start_simulation():
    ...
    About force parameter:
        - When enabled, if simulation is running or completed, it will first stop and clear run logs
        - Cleared contents include: run_state.json, actions.jsonl, simulation.log, etc.
        - Will not clear config file (simulation_config.json) and profile files
        - Suitable for scenarios where simulation needs to be rerun

    About enable_graph_memory_update:
        - When enabled, all Agent activities (posts, comments, likes, etc.) in simulation will be real-time updated to Zep graph
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                # Preparation is completed, check for running processes
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": f"Simulation is running, please call /stop API first to stop it, or use force=true to restart"
                            }), 400
                ...
                return jsonify({
                    "success": False,
                    "error": f"Simulation not prepared, current status: {state.status.value}, please call /prepare API first"
                }), 400
...
def get_run_status(simulation_id: str):
    """
    Get simulation real-time running status (used for frontend polling)
...
def get_run_status_detail(simulation_id: str):
    ...
    Used for frontend displaying real-time dynamics
    ...
    try:
        ...
        # recent_actions only shows content from both platforms in the current latest round
...
def get_simulation_timeline(simulation_id: str):
    ...
    Used for frontend displaying progress bar and timeline view
    
    Query arguments:
        start_round: Starting round (default 0)
        end_round: Ending round (default all)
    
    Returns aggregated info for each round
    """
    try:
...
def get_agent_stats(simulation_id: str):
    ...
    Used for frontend displaying Agent activity ranking, action distribution, etc.
    """
    try:
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (enter wait command mode after completing simulation rounds)
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment not running or closed. Please ensure simulation completed and entered wait command mode."
            }), 400
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // Required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What's your opinion on B?"  // If platform is not specified, default is used
                }
            ],
            "platform": "reddit",              // Optional, default platform (overridden by platform of each item)
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // Required, simulation ID
            "prompt": "What's your overall opinion on this?",  // Required, interview question (same for all Agents)
...
def get_env_status():
    ...
    Checks if simulation environment is alive (can receive Interview commands)
...
def close_simulation_env():
    ...
    Sends close environment command to simulation, making it exit wait command mode gracefully.
    
    Note: This is different from /stop API, /stop will forcefully terminate process,
    while this API will make simulation gracefully close environment and exit.
