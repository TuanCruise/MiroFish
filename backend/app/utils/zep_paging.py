...
Zep node/edge list interface uses UUID cursor paging,
This module encapsulates automatic paging logic (including single-page retry), transparently returning the complete list to the caller.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Single page request, retry with exponential backoff on failure. Only retries transient network/IO errors."""
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Paging to get graph nodes, returning max_items at most (default 2000). Each page request comes with retries."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Paging to get all edges of the graph, returning the complete list. Each page request comes with retries."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
