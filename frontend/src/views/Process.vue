      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">Graph data loading...</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">After generation, the graph construction will automatically start</p>
          </div>
      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After uploading the document,LLMAnalyze document content and automatically generate an ontology structure suitable for public opinion simulation (entity type + Relationship Type)
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Interface Description</div>
                <div class="detail-content">
                  Based on the generated ontology, divide the document into sections and call Zep API Construct a knowledge graph, extracting entities and relationships
                </div>
              </div>
              
              <!-- Waiting for ontology to complete -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">Wait for ontology generation to complete...</div>
              </div>
              ...
              <div class="detail-section" v-if="graphData">
                <div class="detail-label">Construction Result</div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                ...
                <div class="phase-api">Preparing to enter the next step</div>
              </div>
        ...
        <div class="project-panel">
          ...
          <div class="project-details" v-if="projectData">
            ...
            <div class="project-item">
              <span class="item-label">Simulate Requirement</span>
...
// Current ProjectID（Possible From'new'Change to ActualID）
...
const currentPhase = ref(-1) // -1: Uploading, 0: Ontology Generation in Progress, 1: Graph Construction, 2: Complete
...
const goToNextStep = () => {
  ...
  alert('Environment setup function under development...')
}
...
// Initialization - Handle creating a new project or loading an existing project
const initProject = async () => {
  const paramProjectId = route.params.projectId
  
  if (paramProjectId === 'new') {
    // Create new project: from store Obtain data to be uploaded
...
// Process new project - Call ontology/generate API
const handleNewProject = async () => {
  ...
  try {
    ...
    // Invoke ontology generation API
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Continue polling tasks under construction
...
const startBuildGraph = async () => {
  try {
    ...
    buildProgress.value = {
      progress: 0,
      message: 'Graph construction is starting...'
    }
    
    const response = await buildGraph({ project_id: currentProjectId.value })
    
    if (response.success) {
      buildProgress.value.message = 'Graph construction task has started...'
...
const startGraphPolling = () => {
  ...
  // Each 10 Automatically fetch graph data every second
  graphPollTimer = setInterval(async () => {
    await fetchGraphData()
  }, 10000)
}

// Manually refresh knowledge graph
...
const fetchGraphData = async () => {
  try {
    // First retrieve project information to obtain graph_id
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        buildProgress.value = {
          progress: 100,
          message: 'Construction completed, loading the graph...'
        }
...
const renderGraph = () => {
  ...
  if (nodesData.length === 0) {
    ...
    svg.append('text')
      ...
      .attr('fill', '#999')
      .text('Wait for graph data...')
    return
  }
  
  // Create node mapping for name lookup
  const nodeMap = {}
  nodesData.forEach(n => {
    nodeMap[n.uuid] = n
  })
  ...
  simulation.on('tick', () => {
    // Update the position of all edges (including visible lines and transparent clickable areas)
