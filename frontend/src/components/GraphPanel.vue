    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAGReal-time updating of long and short-term memory' : 'Updating in real time...' }}
        </div>
        
        <!-- Prompt after the simulation ends -->
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">There is still a small amount of content being processed; it is recommended to manually refresh the graph later.</span>
          <button class="hint-close-btn" @click="dismissFinishedHint" title="Close the prompt">
      ...
      <div v-else class="graph-state">
        ...
        <p class="empty-text">Waiting for the ontology to be generated...</p>
      </div>
    </div>
...
const showEdgeLabels = ref(true) // Display edge labels by default
const expandedSelfLoops = ref(new Set()) // expanded self-loop term
const showSimulationFinishedHint = ref(false) // Prompt after the simulation ends
const wasSimulating = ref(false) // Track whether it was previously in simulation.

// Turn off the simulation end prompt
const dismissFinishedHint = () => {
  showSimulationFinishedHint.value = false
}

// Listen isSimulating Change, detection simulation ends
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transition from simulation to non-simulation state, display end prompt
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// Toggle self-loop term expansion/Folded state
...
const renderGraph = () => {
  ...
  // Process edge data and calculate the number of edges and their indices between each pair of nodes.
  const edgePairCount = {}
  ...
  tempEdges.forEach(e => {
    if (e.source_node_uuid === e.target_node_uuid) {
      // self-loop - Collected into an array
  ...
  // Record the current edge number being processed for each pair of nodes.
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // Processed self-loop node
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // self-loop edge - Each node adds only one merged self-loop.
    ...
    // Determine whether the direction of the edge is consistent with the standardized direction (sourceUUID < GoalUUID）
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Calculate curvature: when there are multiple edges they spread out, a single edge is a straight line.
    let curvature = 0
    if (totalCount > 1) {
      // Uniformly distributed curvature, ensuring clear distinction.
      // The curvature range increases with the number of edges; the more edges, the larger the curvature range.
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // If the edge direction is opposite to the normalized direction, invert the curvature.
      // This ensures that all edges are distributed within the same reference frame and will not overlap due to differing directions.
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Adjust the distance dynamically based on the number of edges between the pair of nodes.
      // basic distance 150，Each additional edge adds 40
    ...
    // Add a gravitational pull toward the center, causing independent nodes to cluster in the central area.
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw an arc that starts from a node and returns
      const loopRadius = 30
      // Start from the right side of the node, go around and come back.
      const x1 = sx + 8  // starting point offset
      const y1 = sy - 4
      const x2 = sx + 8  // Endpoint offset
      const y2 = sy + 4
      // Draw self-loops using arcs (sweep-flag=1 clockwise)
    ...
    // Offset perpendicular to the line direction, calculated based on distance proportion, ensuring the curve is clearly visible.
    // The more edges there are, the larger the proportion of offset to distance.
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // basic25%，Each additional edge adds5%
  ...
  const getLinkMidpoint = (d) => {
    ...
    // Midpoint of a quadratic Bézier curve t=0.5
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    const pairTotal = d.pairTotal || 1
    ...
    // Quadratic Bézier curve formula B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2, t=0.5
  ...
  const link = linkGroup.selectAll('path')
    ...
    .on('click', (event, d) => {
      ...
      // Reset the style of the previously selected edge
  ...
  // Link labels background (White background makes the text clearer)
  const linkLabelBg = linkGroup.selectAll('rect')
    ...
    .on('click', (event, d) => {
      ...
      linkLabelBg.attr('fill', 'rgba(255,255,255,0.95)')
      linkLabels.attr('fill', '#666')
      // Highlight the corresponding edges
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Only record position, do not restart the simulation (distinguish click and drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect whether the drag has truly started (movement exceeds the threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Only when a genuine drag is detected for the first time will the simulation be restarted.
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only after actually dragging does the simulation gradually stop.
        if (d._isDragging) {
  ...
  // Click the blank area to close the details panel
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
...
// Monitor edge label display toggle
...
/* Prompt style after the simulation ends */
