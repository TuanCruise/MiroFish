            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (Content generation completed, but the entire section may not be finished) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Display special prompts when it is the final answer -->
...
const toggleRawResult = (timestamp, event) => {
  // The position of the save button relative to the viewport
  ...
  // Wait DOM After updating, adjust the scroll position to keep the button in the same place
...
const toolConfig = {
  ...
  'panorama_search': {
    ...
    color: 'blue',
    icon: 'globe' // Globe icon - Represent panoramic search
  },
  'interview_agents': {
  ...
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // Database Icon - Representative Entities
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const queryMatch = text.match(/Analyze Issues:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Extract predictive scenarios
    const reqMatch = text.match(/Predict Scenarios:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // Extract statistical data - Match"Relevant predicted facts: XItem"Format
    ...
    // Extract Sub-questions - Extract completely, no quantity limit
    ...
    // Extract key facts - Extract completely, no quantity limit
    ...
    // Extract core entities - Complete extraction, including summaries and related facts
    const entitySection = text.match(/### 【Core Entity\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
      const entityText = entitySection[1]
      // By "- **" Segment Entity Block
    ...
    // Extract Relationship Chain - Extract completely, no quantity limit
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract current valid facts - Extract completely, no quantity limit
    const activeSection = text.match(/### 【Current Effective Facts[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
    ...
    // Extraction History/Expired Facts - Extract completely, no quantity limit
    ...
    // Extract involved entities - Extract completely, no quantity limit
    const entitySection = text.match(/### 【Involved Entity\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract the number of interviewees (for example "5 / 9 Bit simulationAgent"）
    ...
    // Extract the interviewee's reason for selection
    const reasonMatch = text.match(/### Reason for choosing interviewees\n([\s\S]*?)(?=\n---\n|\n### Interview Transcript)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Parse each person's reason for selection
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // For example: 1. **Alumni_345（index=1）**：As a Wuhan University alumnus...
        ...
        if (name) {
          // The reason for keeping the previous person
          ...
          // Continuation of reasoning (excluding concluding paragraphs)
          currentReason.push(line.trim())
        }
      }
      
      // The reason to save the last person
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract title (such as "Student"、"Education Practitioner" Etc.)
      ...
      if (nameRoleMatch) {
        ...
        // Set the reason for selecting this person
      ...
      if (qMatch) {
        ...
        // Divide the questions by numerical order
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // If there is something before the first question"1."，Requires special handling
      ...
      // Extract Answer - ScoreTwitterAndReddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*Key Quote|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // SeparateTwitterAndRedditAnswer
        ...
        // Platform fallback logic (compatible with old formats: only one platform tag present)
        if (!twitterMatch && redditMatch) {
          // Only Reddit Answers, only copy as default display when it is non-placeholder text
          ...
          // No platform tags (very old format), treat the whole as an answer
      ...
      if (quotesMatch) {
        ...
        // Rollback: Match > "text" or > \u201Ctext\u201D（Chinese Quotation Mark)
        if (!quoteMatches) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/Found\s*(\d+)\s*Item/)
    if (countMatch) result.count = parseInt(countMatch[1])
    
    // Extract relevant facts - Extract completely, no quantity limit
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, 'Predict Scenarios: '),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'The latest key facts associated in sequential memory'),
          ...
          }, expandedFacts.value ? `Collapse ▲` : `Expand All ${props.result.facts.length} Item ▼`)
        ]),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          ...
          }, expandedRelations.value ? `Collapse ▲` : `Expand All ${props.result.relations.length} Item ▼`)
        ]),
        
        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Drift query to generate analysis sub-questions'),
        ...
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'No Relationship Chain')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Current Effective Memory (${props.result.activeFacts.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          }, expandedActive.value ? `Collapse ▲` : `Expand All ${props.result.activeFacts.length} Item ▼`)
        ]),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          }, expandedHistorical.value ? `Collapse ▲` : `Expand All ${props.result.historicalFacts.length} Item ▼`)
        ]),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // For Each Question-Answer regarding maintaining the independent platform selection status
    const platformTabs = reactive({}) // { 'agentIdx-qIdx': 'twitter' | 'reddit' }
    
    // Obtain the current platform selection for a specific question
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "QuestionX：" or "QuestionX:" （Chinese format, back-end new format)
      // 2. "1. " or "\n1. " （Number+Point, compatible with old format)
      let matches = []
      let match

      // Priority Attempt "QuestionX：" Format
      const cnPattern = /(?:^|[\r\n]+)Question(\d+)[：:]\s*/g
    ...
    // Get the answer corresponding to a certain question
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Segmentation successful and index valid
      if (answers.length > 1 && qIdx < answers.length) {
        return answers[qIdx] || ''
      }

      // Split failed: the first question returns the full answer, the rest return empty
      return qIdx === 0 ? answer : ''
    }
    
    // Check whether a question has answers from two platforms (filter placeholder text)
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle button (only shown when there are real answers from both platforms)
...
// Currently active steps (for top display)
const activeStep = computed(() => {
  const steps = workflowSteps.value
  // Currently Found active Steps
  const active = steps.find(s => s.status === 'active')
  if (active) return active
  
  // If not active，Return the Last One done Steps
...
const renderMarkdown = (content) => {
  ...
  // Remove the secondary heading at the beginning (## xxx），Because the chapter title is already displayed on the outer layer
  ...
  // Clean all spaces between list items
  html = html.replace(/<\/li>\s+<li/g, '</li><li')
  // Clean spaces after the list start tag
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Clean spaces before the list end tag
  ...
  // Process line break - Empty lines become paragraph separation, single line breaks become <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Before and after clearing block-level elements <br> Label
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // Clean <p><br> Situation immediately after block-level elements (caused by extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean consecutive <br> Label
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Before the paragraph start tag immediately after cleaning block-level elements <br>
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering of non-consecutive ordered lists: when a single item <ol> Maintain incrementing numbers when separated by paragraph content
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Ensure cleared loading Status
            emit('update-status', 'completed')
            stopPolling()
            // Scrolling logic uniformly after the end of the loop nextTick Processing
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If the task is completed, scroll to the top; otherwise, scroll to the bottom to follow the latest logs
            if (isComplete.value) {
...
// Extract the final answer content - from LLM response Extract chapter content
const extractFinalContent = (response) => {
  if (!response) return null
  
  // Try to extract <final_answer> Content Inside Tags
  ...
  // Try to find Final Answer: Subsequent content (supports multiple formats)
  ...
  // Try to find Final answer: Following content
  ...
  // If the content is long and containsmarkdownformat, try to return after removing thought processes
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove Thought: Initial thought process
...
/* Console Logs - and Step3Simulation.vue Keep consistent */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
