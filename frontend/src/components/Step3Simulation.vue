          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- General fallback: unknown type or have content but not processed as above -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // FromStep2Maximum number of rounds passed in
  minutesPerRound: {
    type: Number,
    default: 30 // Default each round30minutes
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const actionIds = ref(new Set()) // Action used for deduplicationIDSet
const scrollContainer = ref(null)

// Computed
// Display actions in chronological order (the newest at the end, i.e., at the bottom)
...
// TwitterThe platform's simulated elapsed time
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// RedditThe platform's simulated elapsed time
...
const resetAllState = () => {
  ...
  stopPolling()  // Stop any polling that may have existed before
}
...
const doStartSimulation = async () => {
  ...
  // First reset all states to ensure it is not affected by the previous simulation.
  resetAllState()
  ...
  try {
    ...
    addLog('Dynamic graph update mode has been enabled')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Old simulation logs have been cleared, restarting the simulation.')
      }
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      addLog('✓ The simulation has stopped')
      phase.value = 2
      stopPolling()
...
// Track the previous round of each platform to detect changes and output logs.
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect the round changes of each platform separately and output logs.
      ...
      // Check whether the simulation has been completed (passed) runner_status or platform completedState judgment）
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Additional check: if the backend hasn't been updated yet runner_status，But the platform has already reported completion
      // Through detection twitter_completed and reddit_completed State judgment
...
// Check whether all enabled platforms have been completed.
const checkPlatformsCompleted = (data) => {
  ...
  // through actions_count Determine whether the platform is enabled (if count > 0 or running formerly true）
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // use all_actions Get the complete action list
      ...
      // Do not auto-scroll; allow the user to freely view the timeline.
      // The new action will be appended at the bottom.
    }
  } catch (err) {
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('The report generation request has been sent, please wait....')
    return
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Report generation task has started: ${reportId}`)
    ...
    addLog(`✗ Startup report generation exception: ${err.message}`)
    isGeneratingReport.value = false
  }
}
...
onMounted(() => {
  addLog('Step3 Simulated run initialization')
  if (props.simulationId) {
    doStartSimulation()
  }
})
