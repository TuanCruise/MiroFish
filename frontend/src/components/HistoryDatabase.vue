  ...
  >
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <div v-if="project.files.length > 3" class="files-more">
              +{{ project.files.length - 3 }} a file
            </div>
          </div>
          <!-- Placeholder when there is no file -->
          <div class="files-empty" v-else>
            <span class="empty-file-icon">◇</span>
            <span class="empty-file-text">No file available</span>
          </div>
        </div>

        <!-- Card title (before using simulated requirements20Characters as title) -->
        <h3 class="card-title">{{ getSimulationTitle(project.simulation_requirement) }}</h3>

        <!-- Card description (full display of simulated requirements) -->
        ...
        <!-- bottom decorative line (hoverwhen expanded) -->
    ...
    <!-- Historical playback details popup -->
    <Teleport to="body">
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                <div class="modal-label">Simulated requirements</div>
                <div class="modal-requirement">{{ selectedProject.simulation_requirement || 'None' }}</div>
              </div>

              <!-- File list -->
              <div class="modal-section">
                <div class="modal-label">Related file</div>
                ...
                <div class="modal-empty" v-else>No associated file currently</div>
              </div>
            </div>

            <!-- Derivation playback separator -->
            ...
            <!-- Non-replayable prompt -->
            <div class="modal-playback-hint">
              <span class="hint-text">Step3「Start simulation and Step5「Deep interaction needs to be started during operation and does not support historical playback.</span>
            </div>
          </div>
        </div>
      </Transition>
...
const selectedProject = ref(null)  // Currently selected item (for popup)
let observer = null
...
// Card layout configuration - Adjust to a wider aspect ratio
...
const containerStyle = computed(() => {
  ...
  // Calculate the actual required height: number of rows * Card height + (Number of lines-1) * spacing + Small bottom margin
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate the number of cards in the current row, ensuring each row is centered.
    ...
    // Expand downwards, increase the spacing from the title.
    ...
    const x = offset * 35
    // Adjust the starting position, close to the title but maintain appropriate spacing.
...
// Format date (display only the date part)
...
// Generate a title from simulated requirements (take the first20character)
...
// format simulation_id Display (before cropping)6position)
...
// Format round number display (current round/total number of rounds)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update the pending target state (record the latest target state regardless of whether it is in animation)
        pendingState = shouldExpand
        
        // Clear the previous debounce timer (the new scroll intent will override the old one)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // If an animation is in progress, just record the state and handle it after the animation ends.
        if (isAnimating) return
        
        // If the target state is the same as the current state, no processing is needed.
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Use debounce delayed state switching to prevent rapid flickering.
        // The expansion delay is relatively short.(50ms)，The delay when collapsing is relatively long.(200ms)to increase stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check whether the pending execution state still needs to be executed (it may have been overwritten by subsequent scrolling)
          ...
          // Unlock after the animation completes, and check for any pending state changes.
          setTimeout(() => {
            isAnimating = false
            
            // After the animation ends, check if there is a new pending execution state.
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay for a short period before executing to avoid switching too quickly.
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // adjust rootMargin，The bottom of the viewport collapses upward, requiring more scrolling to trigger expansion.
...
onMounted(async () => {
  // Ensure DOM Load data after rendering is complete
  await nextTick()
  await loadHistory()
...
/* useCSSBackground pattern creates a square grid with fixed spacing */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Start positioning from the top left corner; when the height changes, only expand at the bottom without affecting existing grid positions. */
...
.cards-container {
  ...
  /* min-height by JS Dynamic calculation, adaptive based on the number of cards. */
}
...
/* Low-saturation color scheme - Morandicolor scheme */
...
/* ===== Historical playback details popup style ===== */
.modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
